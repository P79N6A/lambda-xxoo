...
@Service
public class OutputAndResourceQuery {

    @Autowired
    private NodeOutputMgr nodeOutputMgr;

    private NodeOutput queryOutputResource(Node node, CmptChar cmptChar, WfFlowNodeOutput output) {

            CharValue charValue = new CharValue(cmptChar, output.getCharValue());
            OutputCharValueHelper.exploreOutputCharValue(node, charValue);
            return new NodeOutput(output, charValue);
    }

    public void queryOutputAndResources(Node node) {
        ...
        for(WfFlowNodeOutput nodeOutput : nodeOutputs) {
            ...
            NodeOutput output = queryOutputResource(node, outputChar, nodeOutput);
