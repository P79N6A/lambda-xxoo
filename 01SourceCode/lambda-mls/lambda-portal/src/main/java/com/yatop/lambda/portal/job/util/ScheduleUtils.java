...
public class ScheduleUtils {
    ...
    public static void createScheduleJob(Scheduler scheduler, com.yatop.lambda.portal.job.domain.Job scheduleJob) {
        try {
            ...
            // 放入参数，运行时的方法可以获取
            jobDetail.getJobDataMap().put(com.yatop.lambda.portal.job.domain.Job.JOB_PARAM_KEY, scheduleJob);

            scheduler.scheduleJob(jobDetail, trigger);

            // 暂停任务
            if (scheduleJob.getStatus().equals(com.yatop.lambda.portal.job.domain.Job.ScheduleStatus.PAUSE.getValue())) {
    ...
    public static void updateScheduleJob(Scheduler scheduler, com.yatop.lambda.portal.job.domain.Job scheduleJob) {
        try {
            ...
            if (trigger == null) {
                ...
                // 参数
                trigger.getJobDataMap().put(com.yatop.lambda.portal.job.domain.Job.JOB_PARAM_KEY, scheduleJob);
            }
