...
public class NodeDeleteQueueMgr extends BaseMgr {
    ...
    public WfFlowNodeDeleteQueue insertNodeDelete(WfFlowNodeDeleteQueue nodeDelete, String operId) {
        if( DataUtil.isNull(nodeDelete) ||
            ...
            throw new LambdaException("Insert node delete failed -- invalid insert data.", "无效插入数据");
        }

        if(checkNodeDelete(nodeDelete.getFlowId(), nodeDelete.getNodeId())) {
            throw new LambdaException("Insert node delete failed -- system error<node already in delete queue>.", "系统错误<节点已在删除队列中>");
        }
        ...
        try {
            ...
            throw new LambdaException("Insert node delete failed.", "插入节点删除失败", e);
        }
    }
    ...
    public int removeNodeDelete(WfFlowNodeDeleteQueue nodeDelete) {
        if(DataUtil.isNull(nodeDelete) || nodeDelete.isFlowIdNotColoured() || nodeDelete.isSequenceNotColoured()){
            throw new LambdaException("Remove node delete -- invalid remove condition.", "无效移除条件");
        }
        ...
        try {
            ...
            throw new LambdaException("Remove node delete failed.", "移除节点删除失败", e);
        }
    }
    ...
    public List<WfFlowNodeDeleteQueue> queryNodeDelete(Long flowId, Long Sequnce) {
        if(DataUtil.isNull(flowId) || DataUtil.isNull(Sequnce)){
            throw new LambdaException("Query node delete failed -- invalid query condition.", "无效查询条件");
        }
        ...
        try {
            ...
            throw new LambdaException("Query node delete failed.", "查询节点删除失败", e);
        }
    }
    ...
    public boolean checkNodeDelete(Long flowId, Long nodeId) {
        if(DataUtil.isNull(flowId) || DataUtil.isNull(nodeId)){
            throw new LambdaException("Check node delete exists failed -- invalid check condition.", "无效检查条件");
        }
        ...
        try {
            ...
            throw new LambdaException("Check node delete exists failed.", "检查节点是否已在删除队列中失败", e);
        }
    }
    ...
    public boolean existsNodeDeleteSequence(Long flowId, Long sequence) {
        ...
        try {
            ...
            throw new LambdaException("Check node delete exists failed.", "检查是否已存在节点删除序号失败", e);
        }
    }
}
